// Karma configuration
// Generated on Fri Sep 25 2015 09:19:10 GMT+0300 (FLE Daylight Time)

module.exports = function(config) {
  config.set({

    // base path that will be used to resolve all patterns (eg. files, exclude)
    basePath: './',


    // frameworks to use
    // available frameworks: https://npmjs.org/browse/keyword/karma-adapter
    frameworks: ['jasmine', 'requirejs'],

    // list of files / patterns to load in the browser
    files: [
	  {pattern: './karma.conf.js', included: false, served: false},
	  
	  {pattern: 'node_modules/jquery/dist/jquery.min.js', included: false, watched: false},
	  {pattern: 'node_modules/bootstrap/dist/js/bootstrap.min.js', included: false, watched: false},
	  {pattern: 'node_modules/angular-route/angular-route.min.js', included: false, watched: false},
	  {pattern: 'node_modules/angular/angular.min.js', included: false, watched: false},
	  {pattern: 'node_modules/angular-ui-bootstrap/ui-bootstrap-tpls.min.js', included: false, watched: false},
      {pattern: 'node_modules/angular-mocks/angular-mocks.js', included: false, watched: false},
      
      {pattern: 'src/scripts/*.js', included: false},
	  {pattern: 'spec/*Spec.js', included: false},
	  {pattern: 'src/test-main.js', included: true},
	],


    // list of files to exclude
    exclude: [
	  'src/scripts/main.js'
    ],


    // preprocess matching files before serving them to the browser
    // available preprocessors: https://npmjs.org/browse/keyword/karma-preprocessor
    preprocessors: {
	  './src/scripts/!(main).js': 'coverage'
    },


    // test results reporter to use
    // possible values: 'dots', 'progress', 'junit', 'coverage'
    // available reporters: https://npmjs.org/browse/keyword/karma-reporter
    reporters: ['dots', 'progress', 'junit', 'coverage'],

	
	junitReporter: {
	  outputDir: 'src/test/junit',
	  outputFile: 'test-results.xml',
	  suite: 'Pracxs Suite',
	},
	coverageReporter: {
	  type: 'html',
	  dir: 'src/test/coverage/',
	},

    // web server port
    port: 9876,


    // enable / disable colors in the output (reporters and logs)
    colors: true,


    // level of logging
    // possible values: config.LOG_DISABLE || config.LOG_ERROR || config.LOG_WARN || config.LOG_INFO || config.LOG_DEBUG
    logLevel: config.LOG_INFO,


    // enable / disable watching file and executing tests whenever any file changes
    autoWatch: true,


    // start these browsers
    // available browser launchers: https://npmjs.org/browse/keyword/karma-launcher
    browsers: ['Chrome', 'PhantomJS'],


    // Continuous Integration mode
    // if true, Karma captures browsers, runs the tests and exits
    singleRun: false,
  })
}
